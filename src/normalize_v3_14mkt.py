...
MARKET_Y5_CAP = {
    ...
    'cleveland':    0.30,
    'louisville':   0.30,
    # 8-market expansion (May 2026) — all COLD, no SOPHI presence
    'ft_wayne':      0.30,
    'cincinnati':    0.30,
    'columbus':      0.30,
    'minneapolis':   0.30,
...
# WAS reincorporation (May 2026): brand-new cold markets cannot organically
# acquire flagship A/B-tier accounts in Y1 because there are no local proof
# points, no warm intros, and operator data is sparse. The penetration curve
# stays the same (Y5 cap = 0.30), but Y1 is restricted to forced-only wins
# (anchors / v7 layers). The cap re-distributes Y1 misses across Y2-Y5.
NEW_COLD_MARKETS_Y1_LOCKOUT = {
    'ft_wayne', 'cincinnati', 'columbus', 'minneapolis', 'st_louis',
...
def base_priority(account):
    ...
    # WAS-dominant priority (May 2026 reincorporation): WAS expresses market
    # readiness / cold-market accessibility. A 1.0 WAS gap ~= $30M TAM gap.
    p += float(account.get('was', 0) or 0) * 300
    p += min(float(account.get('tam', 0) or 0) / 1_000_000.0 * 10, 80)  # ~10x weaker
    return p
...
def run_penetration(accounts, market_key, market_state):
    ...
    for year in range(1, 6):
        ...
        # New-cold-market Y1 lockout: no organic Y1 wins (only forced anchors/v7
        # already processed). Mirrors locked behavior in Phoenix/Denver/Cleveland/
        # Louisville where operator gates produced $0 organic Y1 SOM.
        if year == 1 and market_key in NEW_COLD_MARKETS_Y1_LOCKOUT:
            for a in in_sam:
                if a['acquisition_year'] is None:
                    a['gate_status'] = 'cold-market Y1 lockout: requires proof points / warm intros (deferred to Y2+)'
            continue
...
def main():
    ...
    src_path = here / 'accounts_v2_14mkt.json'
    out_path = here / 'accounts_v3_14mkt.json'
    ...
    # Print summary
    print(f'\n=== v3 PENETRATION ENGINE (14-MARKET) — RESULTS ===\n')
    portfolio_som_yr = {f'y{y}': 0 for y in range(1, 6)}
    all_markets = ['charlotte', 'phoenix', 'denver', 'indianapolis', 'cleveland', 'louisville',
                   'ft_wayne', 'cincinnati', 'columbus', 'minneapolis', 'st_louis',
                   'raleigh', 'charleston', 'ft_lauderdale']
